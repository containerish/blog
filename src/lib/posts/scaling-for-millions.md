<div class="w-full flex justify-center text-primary-500 text-5xl font-semibold">Scaling for millions of users</div>

![scaling for millions](/scaling/scaling.png)
Designing infrastructure and scaling it for many many users is difficult and let's face it, some of us(including me) don't even know how it works under the hood.
So I went reading and wanted to write a blog that explains it in a fun way. I also wanted to make sure it covered the Web 3 infrastructure as well.
...
<img src="/scaling/single-server-setup.svg" alt="" width="600px"/>
...
<div class="not-prose flex flex-col lg:flex-row justify-start items-center gap-10">
<img src="/scaling/single-server-with-db.svg" alt="" width="600px"/>
<ol class="flex flex-col max-w-[600px] gap-2 text-lg"> 
    <li>1. User searches for our website </li>
    <li>2. DNS resolves the name(domain) to its corresponding IP address which user will use to make request to the server </li>
    <li>3. User makes request to the server </li>
...
As the name suggests, scaling up means adding more juice(cpu + memory) to the current server each time the number of users grow a certain number.
This method sounds simple but is often not preferred in production. You see, it still is one server. If it goes down we go down!!!
These servers are expensive, there is a limit to how much you can add to a server and lastly they are difficult to scale down.
Suppose our website attracts a lot more users during holiday season and not as much rest of the year. With Scaled-up architecture, we will be fine during the high trafic season but what happens to the extra server resources rest of the times? We will still be paying for the entire thing.  
This also introduces 'Single Point Of Failure(SPOF)' another concept we should talk about.

  **SPOF** or Single Point Of Failure is a component in an architechture that has a capability of f'ing up entire system. It says - 
``
...
Now for the favorite kind of scaling which is cheaper, efficient and easier to scale down. Scaling out simply refers to adding more servers at need. With increased traffic during holiday season, we will simply add more servers to our web-tier. If one of the servers go down the traffic will be directed to the next available server and in mean time we will spin-up a new server to replace the dead one thus avoiding SPOF.
Similarly, when the traffic goes back to normal, we will remove the extra servers(scale down). We only pay for servers for the time we use them. 

- you don't really care if one of the servers do down
- treat them like cattle
- want to scale on large level and all the time you need cattle not pets
- don't take it personally

## Load Balancer

To be able to distribute the traffic equally to available servers, we need a component called Load Balancer. Load Balancer sits between user and servers(server group) and distributes data across the servers to keep the load on them consistent. Another good thing about laod balancer, it communicates with the servers through a private IP which means the user(malicious ones) cannot get their hands on our servers directly

<div class="not-prose flex flex-col lg:flex-row justify-start items-center gap-10">
<img src="/scaling/multi-server-with-db.svg" alt="" width="800px"/>
<ol class="flex flex-col max-w-[600px] gap-2 text-lg"> 
    <li>1. User searches for our website </li>
    <li>2. DNS resolves the name(domain) to its corresponding IP address which user will use to make request to the server </li>
    <li>3. User makes request to the server which goes through Load Balancer</li>
...
<ol class="flex flex-col max-w-[600px] gap-2 text-lg"> 
    ...
    <li>4. User makes request to server for dynamic content</li>
...
A blockchain is a network of nodes/computers/servers managed by people

I know some of you won't like this. There are many drawbacks you will say. The soultion is not perfect and 1000 more things. This is in no way a guide to scaling. These are but my views. I know its far from perfect but it is evolving everyday. There are some amazing projects in Web3 community which are making it better and better.
But if you are hopeful like I am, there's more where it came from. 